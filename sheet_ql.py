...
class SheetQL:
    ...
    def run_batch(self, config_path: str) -> None:
        ...
        if not YAML_AVAILABLE:
            self.console.print("[bold red]❌ Error: PyYAML is not installed. Please run 'pip install pyyaml' to use batch mode.[/bold red]")
            return
        ...
        try:
            ...
            self.console.print(f"[bold red]❌ Error: Configuration file not found at '{config_path}'[/bold red]")
            return
    ...
    def _prompt_for_paths(self, title: str, filetypes: list[tuple[str, str]], allow_multiple: bool) -> list[str] | None:
        ...
        valid_paths = [p for p in raw_paths if p and os.path.exists(p) and os.path.isfile(p)]
                ...
                try:
                    ...
                    if ext in ['.xlsx', '.xls']:
                        ...
                        for sheet_name in xls.sheet_names:
                            ...
                            table_name = f"{base_name}{ext.replace('.', '_')}_{clean_sheet}"
                        ...
                        self.console.print(f"[yellow]Warning: Unsupported file type '{ext}'. Skipping.[/yellow]")
                        continue
                    
                    all_dataframes[table_name] = df
                except Exception as e:
                    self.console.print(f"[bold red]Error loading '{file_path}': {e}[/bold red]")
        self.console.print(f"[green]✔ Loaded {len(all_dataframes)} table(s) from {len(file_paths)} file(s).[/green]")
    ...
    def _handle_meta_command(self, command_str: str) -> bool:
        ...
        commands: dict[str, Any] = {
            ...
            '.help': self._show_help, '.tables': self._list_tables,
            '.schema': lambda: self._describe_table(parts),
            '.history': self._show_history, '.load': self._add_new_files,
        ...
        if command in commands:
            ...
            if command in ['.exit', '.quit']:
                if self.results_to_save:
                    ...
                    if choice.startswith('y'): self._export_results()
                return True
        else:
    ...
    def _show_help(self) -> None:
        ...
        self.console.print("[yellow].schema <table>[/yellow] - Describe a table's columns and types.")
        self.console.print("[yellow].history[/yellow]      - Show previous queries.")
        self.console.print("[yellow].load[/yellow]         - Load additional data files.")
        self.console.print("[yellow].runscript [path][/yellow]- Execute a YAML script file.")
        self.console.print("[yellow].rename <o> <n>[/yellow]- Rename a table.")
        self.console.print("[yellow].export[/yellow]       - Export staged results to Excel.")
    ...
    def _list_tables(self) -> None:
        ...
        try:
            ...
            for name in tables['name']: self.console.print(f"  - {name}")
        except Exception as e:
    ...
    def _describe_table(self, command_parts: list[str]) -> None:
        ...
        try:
            schema_df = self.db_connection.execute(f'DESCRIBE "{table_name}";').fetchdf()
            table = Table(title=f"Schema for '{table_name}'", header_style="bold magenta")
            for col in schema_df.columns: table.add_column(col.replace('_', ' ').title())
            for _, row in schema_df.iterrows(): table.add_row(*[str(item) for item in row])
            self.console.print(table)
        except Exception as e:
    ...
    def _show_history(self) -> None:
        ...
        if not self.history: self.console.print("  No history yet."); return
    ...
    def _handle_history_rerun(self, command_str: str) -> None:
        ...
        try:
            ...
            if 1 <= index <= len(self.history):
                ...
                self.console.print(f"[red]Error: History index {index} is out of bounds.[/red]")
        except (ValueError, IndexError):
            self.console.print(f"[red]Invalid history command. Use !N where N is a number.[/red]")
    ...
    def _run_script_interactive(self, command_parts: list[str]) -> None:
        ...
        if not script_path:
            ...
            if not paths: self.console.print("[yellow]Script execution cancelled."); return
            script_path = paths[0]
        
        if not YAML_AVAILABLE:
            self.console.print("[bold red]❌ PyYAML not installed.[/bold red]"); return
            
        try:
            with open(script_path, 'r') as f:
                config = yaml.safe_load(f)
            self.console.print(f"[bold cyan]🚀 Executing script: '{script_path}'[/bold cyan]")
            self._execute_yaml_script(config)
            self.console.print(f"[bold green]✔ Script finished. Current tables:[/bold green]")
            self._list_tables()
        except FileNotFoundError:
            self.console.print(f"[bold red]❌ Script file not found: '{script_path}'[/bold red]")
        except yaml.YAMLError as e:
            self.console.print(f"[bold red]❌ Error parsing YAML file: {e}[/bold red]")
    
    def _execute_yaml_script(self, config: dict[str, Any]) -> None:
        """Processes the actions defined in a parsed YAML script."""
        if 'inputs' in config: self._process_yaml_inputs(config.get('inputs', []))
        if 'tasks' in config: self._process_yaml_tasks(config.get('tasks', []))
        if 'export' in config: self._process_yaml_export(config.get('export', {}))
    ...
    def _process_yaml_inputs(self, inputs: list[dict[str, str]]) -> None:
        ...
        if not inputs: self.console.print("[yellow]Inputs section is empty."); return

        paths_to_load = [item['path'] for item in inputs]
        loaded_dfs = self._load_data(paths_to_load)
        alias_map = {os.path.basename(item['path']): item.get('alias') for item in inputs if item.get('alias')}
        ...
        for original_name, df in loaded_dfs.items():
            ...
            if self.db_connection: self.db_connection.register(final_name, df)
    ...
    def _process_yaml_tasks(self, tasks: list[dict[str, str]]) -> None:
        ...
        if not tasks: self.console.print("[yellow]Tasks section is empty."); return
    ...
    def _process_yaml_export(self, export_config: dict[str, str]) -> None:
        ...
        if not export_config: self.console.print("[yellow]Export section is empty."); return
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="SheetQL: Run SQL on local data files.")
    parser.add_argument(
        '-r', '--run',
        dest='config_path',
