...
class TestSheetQL(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with pd.ExcelWriter(self.excel_path) as writer:
            pd.DataFrame({
                "city": ["NY", "LA"], 
                "target": [1000, 500]
            }).to_excel(writer, sheet_name="Q1_Targets", index=False)
    ...
    def test_01_zero_copy_loading(self):
        ...
        tables = self.tool.db_connection.execute("SHOW TABLES").fetchdf()['name'].tolist()
    ...
    def test_04_session_recorder(self):
        ...
        self.assertEqual(self.tool.recorder.transformations[0]['sql'], "SELECT * FROM sales")
    ...
    def test_06_yaml_aliasing_logic(self):
        ...
        config = {
            "inputs": [
                {"path": self.csv_path, "alias": "revenue_data"}
            ]
        }
